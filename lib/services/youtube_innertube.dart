...
class YouTubeInnerTube {
  ...
  static Future<List<YouTubeVideo>> _sendRequest(String token, Map<String, dynamic> payload, String url) async {
    ...
    try {
      ...
      LogService.error(LogService.youtube, 'InnerTube request failed', e);
      rethrow;
    }
  }
      ...
      if (node is Map) {
        ...
        if (renderer != null && renderer is Map<String, dynamic>) {
          ...
          if (videoId != null && videoId.isNotEmpty && !seenIds.contains(videoId)) {
            ...
            if (lengthText != null) {
              final parts = lengthText.split(':').map((e) => int.tryParse(e.trim()) ?? 0).toList();
              if (parts.length == 2) {
            ...
            videos.add(YouTubeVideo(
              ...
              duration: duration,
              thumbnail: thumbnail.isNotEmpty ? thumbnail : 'https://i.ytimg.com/vi/$videoId/hqdefault.jpg',
