...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  bool get _isLivePlayback {
    final source = widget.sourceUrl?.toLowerCase() ?? '';
    return widget.isLiveStream ||
        source.contains('.m3u') ||
        source.contains('/iptv/');
  }
          ...
          return SimpleDialogOption(
            ...
            child: Text(q,
                style: TextStyle(
                  ...
                  fontWeight:
                      q == widget.quality ? FontWeight.bold : FontWeight.normal,
                )),
          );
        }).toList(),
      ),
  ...
  Future<void> _switchQuality(String newQuality) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Не удалось переключить качество: $e')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final currentEp = widget.currentEpisodeIndex >= 0 &&
        ...
        : null;
    final epLabel =
        currentEp != null ? '${currentEp.displayNumber} / $totalEpisodes' : '';
          ...
          child: Center(
            child: Container(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                Text(_reconnectStatus ?? 'Запускаю ${widget.quality}...',
  ...
  Widget _buildTopBar(
      ...
      List<SeriesEpisode> filteredEpisodes) {
    final filteredIndex = filteredEpisodes.indexWhere((e) =>
        e.link ==
        (widget.currentEpisodeIndex < (widget.episodes?.length ?? 0)
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        if (hasEpisodes && filteredEpisodes.length > 1)
          GestureDetector(
              onTap: () => _showEpisodeList(filteredEpisodes, filteredIndex),
              child: const Padding(
                  padding: EdgeInsets.all(8),
                  child: Icon(Icons.list, color: Colors.white70, size: 22))),
        ...
        if (translations.length > 1)
          GestureDetector(
            onTap: () => _showTranslationList(translations, currentTranslation),
            child: Padding(
                padding: const EdgeInsets.symmetric(horizontal: 6),
                child: Text(
                    currentTranslation.length > 10
                        ? '${currentTranslation.substring(0, 10)}\u2026'
                        : currentTranslation,
                    style:
      ...
      builder: (_) => SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (_, i) {
              final ep = episodes[i];
              final globalIdx =
                  widget.episodes?.indexWhere((e) => e.link == ep.link) ?? -1;
              return ListTile(
                title: Text(ep.displayNumber,
                    style: TextStyle(
                        color:
                            i == currentIdx ? Colors.orange : Colors.white70)),
                subtitle: Text(ep.title,
                    style:
                        const TextStyle(color: Colors.white54, fontSize: 12)),
                trailing: i == currentIdx
                    ? const Icon(Icons.play_arrow,
                        color: Colors.orange, size: 18)
                    : null,
                onTap: () {
                  Navigator.pop(context);
                  if (globalIdx >= 0) widget.onEpisodeChange?.call(globalIdx);
                },
              );
            },
          )),
      ...
      builder: (_) => SizedBox(
          ...
          child: ListView(
            ...
            final matchingIdx = widget.episodes?.indexWhere((e) =>
                    ...
                    (e.episode ?? '') == currentEpNum) ??
                -1;
            return ListTile(
              title: Text(t,
                  style: TextStyle(
                      color: t == current ? Colors.orange : Colors.white70,
                      fontWeight:
                          t == current ? FontWeight.bold : FontWeight.normal)),
              trailing: t == current
                  ? const Icon(Icons.check, color: Colors.orange, size: 18)
                  : null,
              onTap: () {
                Navigator.pop(context);
                if (matchingIdx >= 0) widget.onEpisodeChange?.call(matchingIdx);
              },
            );
          }).toList())),
    );
